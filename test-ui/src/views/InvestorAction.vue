...
export default {
  ...
  methods: {
    ...
    async mintVested() {    
      console.log(`Mint Vested: ${this.bmtVestedToMint}, time: ${this.bmtVestingDate}`);

      const unixTime = Date.parse(this.bmtVestingDate + 'T00:00:00Z') / 1000;
      console.log(unixTime);
      console.log("OK " + blockchain.toHumanDate(unixTime));
      await blockchain.mintVested(this.investor.address, this.bmtVestedToMint, unixTime);
    }
  },
  async created() {}
};
